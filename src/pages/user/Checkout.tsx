...
export const Checkout = () => {
    ...
    const handleConfirmPayment = async () => {
        ...
        try {
            ...
            if (proofFile) {
                const { url, error: uploadError } = await storageService.uploadPaymentProof(proofFile);
                if (uploadError || !url) throw new Error(uploadError || 'فشل رفع صورة إيصال التحويل');
                proofImageUrl = url;
            }

            // 2. Add to Enrollments with status
            const status = (course.is_free || course.price === 0) ? 'approved' : 'pending';
            const { error: enrollmentError } = await enrollmentService.enrollInCourse(id!, proofImageUrl, status);

            if (enrollmentError) {
                if (enrollmentError.includes('23505') || enrollmentError.includes('already enrolled') || enrollmentError.includes('مشترك بالفعل')) {
                    toast.error('أنت مشترك بالفعل أو لديك طلب معلق لهذا الكورس');
                    return;
                }
                throw new Error(enrollmentError);
            }
                    ...
                    >
                        ...
                        <div className="p-8 space-y-5">
                            ...
                            <button
                                ...
                                className="w-full mt-8 py-5 bg-brand-primary text-white rounded-[var(--radius-button)] font-black text-lg shadow-xl hover:bg-brand-primary-hover active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-brand-primary disabled:active:scale-100 transition-all flex items-center justify-center gap-3 group"
                            >
                                {isProcessing ? (
